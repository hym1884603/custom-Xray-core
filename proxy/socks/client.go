...
func (c *Client) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	if user != nil {
		...
		if inbound != nil && inbound.Name == "vmess" {
			uuid := inbound.User.Account.(*vmess.MemoryAccount).ID.String()
			if account, ok := user.Account.(*Account); ok {
				print(account.Username + "\n")
				account.Username = ReplaceSessionSuffix(account.Username, uuid)
			}
		}
		request.User = user
		if account, ok := user.Account.(*Account); ok {
			print("user2:" + account.Username + "\n")
		}
...
// 替换最后一个 "-" 之后的内容为清理后的 UUID
func ReplaceSessionSuffix(originalStr, uuid string) string {
	// 1. 找到最后一个 "-" 的位置
	lastDashIndex := strings.LastIndex(originalStr, "-")
	if lastDashIndex == -1 {
		return originalStr // 如果没有找到则直接返回原字符串
	}

	// 2. 清理 UUID 的连字符
	cleanedUUID := strings.ReplaceAll(uuid, "-", "")

	// 3. 拼接新字符串（保留最后一个 "-" 后追加清理后的 UUID）
	return originalStr[:lastDashIndex+1] + cleanedUUID
}
